<?php

/*
 * This file is part of the ONGR package.
 *
 * (c) NFQ Technologies UAB <info@nfq.com>
 *
 * For the full copyright and license information, please view the LICENSE
 * file that was distributed with this source code.
 */

namespace ONGR\DeepLinkBundle\Tests\Functional\Service\Modifier;

use ONGR\DeepLinkBundle\Service\Modifier\DocumentParameterInject;
use ONGR\DeepLinkBundle\Tests\Unit\TestDocument;

class DocumentParameterInjectTest extends \PHPUnit_Framework_TestCase
{
    /**
     * @return array
     */
    public function getTestModifyData()
    {
        $out = [];

        // Case #0.
        $link = 'http://example.com/offer/123';
        $config = ['partner' => 'p'];
        $doc = new TestDocument;
        $doc->partner = 23;
        $expected = 'http://example.com/offer/123?p=23';
        $out[] = [$link, $config, $doc, $expected];

        // Case #1.
        $link = 'http://example.com/offer/123';
        $config = ['partner' => 'p', 'super' => 's'];
        $doc = new TestDocument;
        $doc->partner = 23;
        $expected = 'http://example.com/offer/123?p=23';
        $out[] = [$link, $config, $doc, $expected];

        // Case #2.
        $link = 'http://example.com/offer/123?search=ongr';
        $config = ['partner' => 'p', 'super' => 's'];
        $doc = new TestDocument;
        $doc->partner = 23;
        $expected = 'http://example.com/offer/123?search=ongr&p=23';
        $out[] = [$link, $config, $doc, $expected];

        // Case #3.
        $link = 'http://example.com/offer/123?search=ongr';
        $config = ['partner' => 'p', 'super' => 's'];
        $doc = new TestDocument;
        $doc->partner = 23;
        $doc->super = 'deeplink';
        $expected = 'http://example.com/offer/123?search=ongr&p=23&s=deeplink';
        $out[] = [$link, $config, $doc, $expected];

        // Case #4.
        $link = 'http://example.com/offer/123';
        $config = ['partner' => 'p', 'id' => 'id'];
        $doc = new TestDocument;
        $doc->partner = 23;
        $doc->setId(456);
        $expected = 'http://example.com/offer/123?p=23&id=456';
        $out[] = [$link, $config, $doc, $expected];

        return $out;
    }

    /**
     * Check if modifying works as expected.
     *
     * @param string       $link
     * @param array        $config
     * @param TestDocument $doc
     * @param string       $expected
     *
     * @dataProvider getTestModifyData()
     */
    public function testModify($link, $config, $doc, $expected)
    {
        $service = new DocumentParameterInject($config);

        $params = [];
        $this->assertEquals($expected, $service->modify($link, $doc, $params));
    }
}
